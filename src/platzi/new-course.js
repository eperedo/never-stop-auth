...
async function getAgenda() {
	...
	const agenda = await page.evaluate(() => {
		...
		const agendaItems = window.data.scheduleItems.agenda_all.agenda_courses;
		const agendaValues = Object.values(agendaItems).filter((x) =>
			x.launch_date.includes(formatDate),
		);
		return agendaValues;
	});
